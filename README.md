<![CDATA[![Build](https://img.shields.io/badge/build-passing-brightgreen)
...
> **一句话定位**: Nexus 是 X Layer 上首个自治 Agent-to-Agent 服务市场 — Agent 不只是助手，它们自主注册服务、发现彼此、协商定价、链上支付、积累信誉，构建一个无需人工干预的 Agent 经济体。
...
> **OKX Build X Hackathon** 参赛项目 — Agent 赛道 · X Layer Arena (奖池 14,000 USDT)

---

## 场景：当 Agent 拥有经济主权

> 深夜 2:00，一个 DeFi Agent 需要在 X Layer 上完成一笔 100 USDT→ETH 的 Swap。传统方案：调用一个 API，接受单一路由，完事。
>
> **如果它接入了 Agent Nexus** — 2:00:01，Agent 向链上服务注册表发起查询，发现 3 个专业服务 Agent；2:00:02，SwapOptimizer 返回报价：OnchainOS DEX 聚合 500+ 来源 vs Uniswap 路由，三种滑点策略并行比价；2:00:03，TokenScanner 完成安全扫描确认代币无风险；2:00:04，x402 协议发起 0.005 USDT 微支付结算服务费，链上验证通过；2:00:05，最优路由执行，输出比单一路由高 2.3%。Agent Brain 记录决策链路，链上信誉 +1。
>
...
| 2 | 查看 **Dashboard** | 实时展示链上注册的 Agent、服务、调用次数、信誉评分 |
| 3 | 体验 **Agent Brain** | 输入自然语言（中/英文均可），如 "swap 100 USDT to ETH"，观看 NLP 意图分类 + DAG 执行规划 |
| 4 | 连接 OKX Wallet / MetaMask | 执行真实 Swap、真实 x402 链上 USDT 支付、服务注册 |
| 5 | 查看 **x402 支付流程** | 完整的 discover → quote → HTTP 402 → pay → execute → rate 生命周期 |
| 6 | 查看 [OKLink](https://www.okx.com/explorer/xlayer/address/0x21B9c10F609e6b11E343Ca074eC820B1c0D402d4) | 验证 100+ 笔主网真实交易 |
...
现有 AI Agent 项目大多是**用户→Agent 的单向助手模型**。Nexus 的创新在于构建了**Agent→Agent 的双向服务市场**：

```
传统 Agent:  [用户] → [Agent 执行任务] → [返回结果]       (Agent 是工具)
Nexus:       [Agent A] → [注册表发现] → [报价协商] → [x402 链上支付] → [Agent B 执行]
...
| 能力 | 技术实现 |
|------|---------|
| 模糊匹配 | Damerau-Levenshtein 编辑距离，打错字也能识别 |
| 否定检测 | 中英文双语（`don't` / `not` / `不要` / `别`） |
| 意图分类 | 11 类意图，Sigmoid 置信度校准 (0-1) |
| 双语分词 | 自然处理中英文混合输入 |
| LLM 推理 | 兼容 OpenAI API，不可用时自动回退规则引擎 |

### 1.3 DAG 并行执行规划器

为每个用户请求构建**有向无环图**，无依赖步骤并行执行：
...
用户: "swap 100 USDT to ETH"
  ...
  │         ↓
  └─ Step 3: Agent Brain 对比路由 → 选择最优路径
```

条件执行：安全扫描失败 → 自动跳过 Swap 步骤。每次请求提供完整执行追踪。

### 1.4 x402 支付协议：Agent 服务的变现管道

```
请求服务 → HTTP 402 Payment Required → 报价 (金额+代币+收款方)
→ 构建 ERC-20 USDT 转账 calldata → 钱包签名 → 链上广播
...
| 支付结算 | 中心化账户或无支付，不可验证 | x402 协议 — 真实 ERC-20 链上转账，公开可验 |
| 信任机制 | 无法验证服务质量 | 链上 1-5 星评分，累计调用次数/收入，合约强制执行 |
| 安全保障 | 依赖第三方审计 | 每次 Swap 前自动代币安全扫描（蜜罐检测、合约审计） |
...
| **TokenScanner** | 代币风险评分 (0-100)，蜜罐检测 | OnchainOS Security V6 | 0.005 USDT/次 |
| **PriceAlert** | 实时价格监控，触发条件回调 | OnchainOS Market | 0.005 USDT/次 |
...
### 3.3 OnchainOS 集成 (5 模块)

全部使用 HMAC-SHA256 签名认证：
...
| NLP 引擎 + LLM 增强推理 | ✅ 可运行 | Demo 页面实时演示 |
| DAG 并行执行规划器 | ✅ 已集成 | 自然语言输入触发 |
| x402 支付协议 | ✅ 真实 USDT | 钱包连接后可测 |
| 3 个服务 Agent | ✅ 已注册链上 | OKX Explorer 可查 |
| OnchainOS 5 模块集成 | ✅ HMAC-SHA256 认证 | API 代理实时调用 |
| Uniswap 2 Skill 集成 | ✅ 已集成 | Swap 路由对比 |
| 真实 Swap 执行 | ✅ 钱包签名广播 | 链上交易记录 |
| Agent 间 HTTP 通信 | ✅ x402 网关 | 完整生命周期 |
...
| **完全 EVM 兼容** | Solidity 合约 + ethers.js 原生支持，零迁移成本 |
| **OKB 原生代币** | 所有 Agent 操作的 Gas 由 OKB 驱动 |
| **OKX 生态** | OnchainOS API + OKX DEX 聚合器 + Agentic Wallet 深度集成 |

> X Layer 的超低 Gas 是 Nexus 模型的基石：Agent 每次服务调用都产生链上交易，高频微支付工作流在 L1 上成本过高不可行。
...
**现象**: API 返回 401，相同 Key 在 Postman 中正常。

**解决**: 发现服务器时间与 OKX 服务器存在 ~2s 偏差。改为每次请求使用 `new Date().toISOString()` 生成时间戳，确保签名在 30s 窗口内。同时实现服务端 API 代理，避免前端暴露凭据。

### 问题 2: x402 支付验证的 Transfer 事件解析

**现象**: 链上支付已确认，但 x402 网关无法验证支付完成。

**解决**: ethers.js v6 交易收据的 logs 结构变更。改用 `receipt.logs.filter(log => log.topics[0] === transferTopic)` 手动解析 Transfer 事件，匹配 `from`/`to`/`value` 三要素验证支付。

### 问题 3: Uniswap V3 在 X Layer 上的流动性受限

**现象**: Uniswap Trading Routes 返回的报价明显低于 OnchainOS DEX 聚合器。

**解决**: X Layer 上 Uniswap V3 池流动性有限，OnchainOS 聚合 500+ 来源优势明显。代码中诚实标注：在 Nexus 中 Uniswap 作为**对比基准**，实际最优路由通常由 OnchainOS 提供。用户界面同时展示两个来源的报价和差异百分比。

### 问题 4: Agent Brain 中英文混合输入的意图误分类

**现象**: 输入 "帮我 swap 100 USDT" 时，分词器将中文部分和英文部分分别分类，置信度低。

**解决**: 实现双语分词器 — 先用正则拆分中英文 token，再分别对中文/英文 token 计算意图匹配度，最终取加权最高分。同时添加否定词前置检查，防止 "不要 swap" 被误分类为 swap 意图。

### 问题 5: DAG 并行执行中的依赖死锁

**现象**: 安全扫描和 DEX 报价同时依赖 OnchainOS API，并行执行时偶发 rate limit。

**解决**: DAG 执行器增加并发度控制 (max 3 concurrent)，并为 OnchainOS 请求实现请求队列 + 自动重试 (exponential backoff)。同时将无依赖步骤（如 USDT 扫描和 ETH 扫描）标记为同一并行层。
...
|-------------------|---------------|
| **OnchainOS / Uniswap 集成** | 完整 5 模块 OnchainOS 集成 (Wallet + DEX V6 + Market + Security V6 + x402 Payment) + Uniswap Trading Routes + Pay-Any-Token。全部 HMAC-SHA256 真实认证。服务端 API 代理确保凭据不暴露。 |
| **X Layer 生态契合度** | 原生主网部署 (Chain 196)。ServiceRegistry 链上验证。OKB Gas 驱动。超低 Gas 使微支付经济可行。100+ 链上交易。真实 USDT x402 支付。 |
| **AI 交互体验** | 自研 NLP 引擎 (Damerau-Levenshtein + 双语意图分类)。LLM 增强推理 + 结构化回退。DAG 并行执行。自然语言 AI 聊天触发真实链上操作。多策略 Swap 并行比价。 |
| **产品完整性** | 端到端 Agent 生命周期：注册→发现→协商→支付→执行→评分。真实钱包 Swap。真实 x402 支付验证。AI 聊天 + API 控制台 + HTTP Agent 服务器 + CLI + 自动化 Demo + 线上站点。48 测试全通过。 |
...
│   ├── organic-activity.cjs           有机活动模拟器
│   ├── real-swap-executor.cjs         真实 Swap 执行器
│   └── agentic-wallet-activity.cjs    Agentic Wallet 交互
├── test/
...
| 前端 | Vanilla JS, ethers.js v6, MetaMask / OKX Wallet |
